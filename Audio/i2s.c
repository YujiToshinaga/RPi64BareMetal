...
void i2s_init(int master_mode, int sr, int bit)
{
    ...
    if (master_mode == I2S_MSTR) {
        ...
        case 32:
            ...
            wr_word(PCM_CS_A,
                    ...
                    PCM_CS_RXTHR_LSTFL | PCM_CS_TXTHR_LSSFL |
     ...
     *  割込みを有効にする
     */
    exception_int_ena(IRQNO_PCM, i2s_isr);
}
...
bool_t i2s_rcv_isrdy(void)
{
    ...
    if ((rd_word(PCM_CS_A) & PCM_CS_RXD_BIT)
            == PCM_CS_RXD_BIT) {
        ready = true;
    } else {
        ready = false;
...
bool_t i2s_snd_isrdy(void)
{
    ...
    if ((rd_word(PCM_CS_A) & PCM_CS_TXD_BIT)
            == PCM_CS_TXD_BIT) {
        ready = true;
    } else {
        ready = false;
...
//    tmp = rd_word(PCM_CS_A);
//    tmp |= PCM_CS_SYNC_BIT | PCM_CS_RXERR_BIT;
//    tmp |= PCM_CS_RXTHR_LSTFL;
//    tmp |= PCM_CS_RXCLR_BIT;
//    wr_word(PCM_CS_A, tmp);

    tmp = rd_word(PCM_INTEN_A);
    tmp |= PCM_INTEN_RXR_BIT;
    wr_word(PCM_INTEN_A, tmp);
//
//    tmp = rd_word(PCM_INTSTC_A);
//    tmp |= PCM_INTEN_RXR_BIT;
//    wr_word(PCM_INTSTC_A, tmp);
}
...
//    tmp = rd_word(PCM_CS_A);
//    tmp |= PCM_CS_SYNC_BIT | PCM_CS_TXERR_BIT;
//    tmp |= PCM_CS_TXTHR_LSSFL;
//    tmp |= PCM_CS_TXCLR_BIT;
//    wr_word(PCM_CS_A, tmp);
...
//
//    tmp = rd_word(PCM_INTSTC_A);
//    tmp |= PCM_INTSTC_TXW_BIT;
//    wr_word(PCM_INTSTC_A, tmp);
}

void i2s_isr(void)
{
    if ((rd_word(PCM_INTSTC_A) & PCM_INTSTC_RXR_BIT)
            == PCM_INTSTC_RXR_BIT) {
        i2s_rcv_cb();
        wr_word(PCM_INTSTC_A, rd_word(PCM_INTSTC_A) | PCM_INTSTC_RXR_BIT);
    }
    if ((rd_word(PCM_INTSTC_A) & PCM_INTSTC_TXW_BIT)
            == PCM_INTSTC_TXW_BIT) {
        i2s_snd_cb();
        wr_word(PCM_INTSTC_A, rd_word(PCM_INTSTC_A) | PCM_INTSTC_TXW_BIT);
    }
}
